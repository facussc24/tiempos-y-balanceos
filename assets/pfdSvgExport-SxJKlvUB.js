import{P as q,S as nt}from"./pfdTypes-Ck1MTug8.js";import{sanitizeFilename as it}from"./filenameSanitization-D6J-gYyb.js";import{g as rt}from"./ppeBase64-Bv0ey7tA.js";const S=440,J=62,V=70,W=32,I=44,j=32,R=20,H=190,st=48,lt=H+32,Z=135,Q=75,c="Inter,Arial,Helvetica,sans-serif",x="#374151",u="#111827",O={border:x,bg:"#FFFFFF",bgDark:"#F3F4F6",text:u},at={operation:O,transport:O,inspection:O,storage:O,delay:O,decision:O,combined:O};function X(t){let e=t.stepType==="decision"?V:J;return t.productCharacteristic&&(e+=16),t.processCharacteristic&&(e+=12),t.rejectDisposition!=="none"&&(t.stepType==="inspection"||t.stepType==="combined"||t.stepType==="decision")&&(e+=18),e}function ct(){let t=`<defs>
    ...
    <marker id="reworkArrowMarker" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto-start-reverse">
        <path d="M 0 0.5 L 10 5 L 0 9.5 z" fill="${u}"/>
            ...
            </g>`}}function h(t){return t?String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function M(t,e){return t.length<=e?t:t.slice(0,e-1)+"…"}function $t(t){const e=[];let o=0;for(;o<t.length;)if(t[o].branchId){const n={},i=[];for(;o<t.length&&t[o].branchId;){const s=t[o].branchId;n[s]||(n[s]=[],i.push(s)),n[s].push(t[o]),o++}const r=i.map(s=>({branchId:s,label:n[s][0].branchLabel||`Línea ${s}`,steps:n[s]}));e.push({type:"parallel",steps:[],branches:r})}else{const n=[];for(;o<t.length&&!t[o].branchId;)n.push(t[o]),o++;e.push({type:"main",steps:n})}return e}function ft(t){let e=S;for(const o of t)if(o.type==="parallel"&&o.branches){const n=o.branches.length,i=S+R*2,r=n*i+(n-1)*j;r>e&&(e=r)}return e}function K(t,e,o){const n=at[t.stepType],i=X(t),r=t.stepType==="decision"?V:J,s=e+R+W/2,$=o+r/2,d=e+R+W+12,a=o+r/2,l=t.stepType==="decision",g=l?2.5:2,w="url(#dropShadow)",y=t.productSpecialChar==="CC"||t.processSpecialChar==="CC",A=!y&&(t.productSpecialChar==="SC"||t.processSpecialChar==="SC"),L=t.isExternalProcess;let p="",f=e+S-R;if(y&&(f-=28,p+=`<rect x="${f}" y="${o+10}" width="24" height="16" rx="8" fill="white" stroke="${x}" stroke-width="1.5"/>
            ...
            <text x="${f+m/2}" y="${o+22}" font-size="8" font-weight="bold" fill="${x}" text-anchor="middle" font-family="${c}" letter-spacing="0.3">${k}</text>`}const b=[];t.machineDeviceTool&&b.push(t.machineDeviceTool),t.department&&b.push(t.department);const F=b.length>0?`<text x="${d}" y="${a+15}" font-size="9" fill="#9CA3AF" font-family="${c}">${h(M(b.join(" · "),50))}</text>`:"";let C="",T=b.length>0?a+28:a+16;t.productCharacteristic&&(C+=`<text x="${d}" y="${T}" font-size="8" fill="#6B7280" font-family="${c}"><tspan font-weight="600">Prod:</tspan> ${h(M(t.productCharacteristic,42))}</text>`,T+=12),t.processCharacteristic&&(C+=`<text x="${d}" y="${T}" font-size="8" fill="#6B7280" font-family="${c}"><tspan font-weight="600">Proc:</tspan> ${h(M(t.processCharacteristic,42))}</text>`);let z="";if(t.linkedAmfeOperationId||t.linkedCpItemIds&&t.linkedCpItemIds.length>0){let k=e+S-R;const m=o+i-18;t.linkedCpItemIds&&t.linkedCpItemIds.length>0&&(k-=30,z+=`<rect x="${k}" y="${m}" width="26" height="14" rx="7" fill="#F3F4F6" stroke="#9CA3AF" stroke-width="0.75"/>
                ...
                <text x="${k+18}" y="${m+10}" font-size="7" font-weight="600" fill="${x}" text-anchor="middle" font-family="${c}">AMFE</text>`)}const _=t.notes?`<text x="${e+S-8}" y="${o+14}" font-size="12" font-weight="bold" fill="${x}" text-anchor="end" font-family="${c}">*</text>`:"",v=y||A||L?40:52,E=y?`<rect x="${e}" y="${o+2}" width="3" height="${i-4}" rx="1.5" fill="${u}"/>`:"";let D="";if(t.rejectDisposition!=="none"&&(t.stepType==="inspection"||t.stepType==="combined"||t.stepType==="decision")){const k=o+i-16,m=e+R+W+12,P=l?"NOK → ":"",N=l?`<text x="${e+S-R-4}" y="${k}" font-size="8" font-weight="700" fill="#16A34A" font-family="${c}" text-anchor="end">OK ↓</text>`:"";t.rejectDisposition==="rework"?D=`<g class="pfd-disp-rework">
                ...
                ${N}
            </g>`)}const B=[`${t.stepNumber} — ${t.description}`];t.machineDeviceTool&&B.push(`Equipo: ${t.machineDeviceTool}`),t.department&&B.push(`Área: ${t.department}`),t.productCharacteristic&&B.push(`Caract. Producto: ${t.productCharacteristic}`),t.processCharacteristic&&B.push(`Caract. Proceso: ${t.processCharacteristic}`),t.rejectDisposition!=="none"&&B.push(`Disposición: ${t.rejectDisposition}`),t.notes&&B.push(`Nota: ${t.notes}`);const Y=`<title>${h(B.join(`
...
`).length:0,A=y>1?` (+${y-1} más)`:"",L=d+6,p=20,f="11.5";return`<g class="pfd-header">
        ...
        <text x="16" y="${L+12+p}" font-size="${f}" fill="${u}" font-family="${c}">
            <tspan font-weight="700">Elaboró:</tspan> ${h(n.preparedBy)}${n.preparedDate?` (${h(n.preparedDate)})`:""}  |  <tspan font-weight="700">Aprobó:</tspan> ${h(n.approvedBy)}${n.approvedDate?` (${h(n.approvedDate)})`:""}  |  <tspan font-weight="700">Cód. Prov.:</tspan> ${h(n.supplierCode)}  |  <tspan font-weight="700">Equipo:</tspan> ${h(M(n.coreTeam,60))}
        </text>
        ...
        </svg>`;const n=$t(t.steps),i=Q+q.length*Z,s=Math.max(ft(n),i)+st*2,$=s/2,d=$-S/2,a=[];let l=lt;if(t.header.linkedAmfeId||t.header.linkedCpId){const p=mt($,l,s,t);p&&(a.push(p),l+=36)}const g=new Map,w=new Map;t.steps.forEach((p,f)=>w.set(p,f));for(let p=0;p<n.length;p++){const f=n[p];if(p>0&&(a.push(U($,l,l+I)),l+=I),f.type==="main")for(let b=0;b<f.steps.length;b++){b>0&&(a.push(U($,l,l+I)),l+=I);const F=f.steps[b];a.push(K(F,d,l));const C=F.stepNumber.trim()||`__idx_${w.get(F)}`;g.set(C,{stepNumber:F.stepNumber,x:$,y:l,h:X(F),nodeX:d}),l+=X(F)}else if(f.type==="parallel"&&f.branches){const b=f.branches,F=b.length,C=S+R*2,G=F*C+(F-1)*j,T=$-G/2;a.push(`<text x="${$}" y="${l+12}" font-size="10" font-weight="700" fill="${x}" text-anchor="middle" font-family="${c}">FLUJO PARALELO</text>`),l+=20;const z=[];for(let v=0;v<F;v++){const E=T+v*(C+j);z.push({cx:E+C/2})}a.push(pt($,l,z)),l+=20;let _=0;for(const v of b){let E=30;for(const D of v.steps)E+=X(D);E+=(v.steps.length-1)*I,E>_&&(_=E)}for(let v=0;v<b.length;v++){const E=b[v],D=T+v*(C+j);a.push(`<rect x="${D}" y="${l}" width="${C}" height="${_}" rx="4" fill="#F9FAFB" stroke="#D1D5DB" stroke-width="1"/>`);const B=h(E.label),Y=B.length*7+20,k=D+C/2-Y/2;a.push(`<rect x="${k}" y="${l+6}" width="${Y}" height="18" rx="9" fill="white" stroke="#9CA3AF" stroke-width="1"/>
                    <text x="${D+C/2}" y="${l+19}" font-size="10" font-weight="700" fill="${u}" text-anchor="middle" font-family="${c}">${B}</text>`);let m=l+30;for(let P=0;P<E.steps.length;P++){if(P>0){const ot=D+C/2;a.push(U(ot,m,m+I)),m+=I}const N=E.steps[P];a.push(K(N,D+R,m));const et=N.stepNumber.trim()||`__idx_${w.get(N)}`;g.set(et,{stepNumber:N.stepNumber,x:D+C/2,y:m,h:X(N),nodeX:D+R}),m+=X(N)}}l+=_,a.push(xt($,l+4,z)),l+=18,a.push(`<text x="${$}" y="${l+14}" font-size="10" font-weight="700" fill="${x}" text-anchor="middle" font-family="${c}">CONVERGENCIA</text>`),l+=22}}const y=gt(t.steps,g,s);if(y&&a.push(y),!o?.skipNotes){const p=t.steps.filter(f=>f.notes);if(p.length>0){l+=32;const f=ut($,l,s,p);f.svg&&(a.push(f.svg),l+=f.height)}}l+=48,a.push(dt($,l)),l+=36,l+=16,a.push(wt($,l,s)),l+=50;const A=l+20,L=ht(t,s,e);return`<?xml version="1.0" encoding="UTF-8"?>

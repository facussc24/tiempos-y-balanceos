/**
 * _auditArmrest.mjs — Audit script for Armrest Door Panel Patagonia AMFE
 * ID: 5268704d-30ae-48f3-ad05-8402a6ded7fe
 *
 * Checks:
...
const docLabel = [row.project_name, row.subject, row.part_number].filter(Boolean).join(' | ') || 'Armrest Door Panel Patagonia';
...
for (const op of ops) {
    ...
    // Check for English words in operation name
...
const totalWEs = ops.reduce((sum, op) => sum + (op.workElements || []).length, 0);

console.log(`\n${'='.repeat(100)}`);
console.log(`  AUDIT: Armrest Door Panel Patagonia AMFE`);
console.log(`  Document: ${docLabel}`);
console.log(`  ID: ${AMFE_ID}`);
console.log(`  Date: ${new Date().toISOString().slice(0, 10)}`);
console.log(`  Operations: ${ops.length} | WEs: ${totalWEs} | Causes: ${totalCauses}`);

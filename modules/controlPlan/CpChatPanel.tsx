/**
 * Control Plan Chat Copilot Panel
 *
 * Chat interface where the user gives natural language instructions
 * and Gemini modifies the Control Plan document directly.
 * Emerald/green theme to match CP module style.
 */
...
import {
    ...
    CpChatExecutionResult, ChatMessage,
} from './cpChatEngine';
...
interface UIMessage {
    ...
    executionResult?: CpChatExecutionResult;
    questions?: string[];
    timestamp: number;
}

interface CpChatPanelProps {
    doc: ControlPlanDocument;
    amfeDoc?: AmfeDocument;
    onApplyChanges: (newDoc: ControlPlanDocument) => void;
    onClose: () => void;
...
const ACTION_ICONS: Record<string, React.ReactNode> = {
    ...
    remove: <Trash2 size={11} className="text-red-500" />,
    bulk: <RefreshCw size={11} className="text-amber-600" />,
    validate: <ShieldCheck size={11} className="text-indigo-600" />,
};

function ActionCard({ action }: { action: CpChatAction }) {
    const icon = action.action === 'addItem' ? ACTION_ICONS.add
        : action.action === 'removeItem' ? ACTION_ICONS.remove
        : action.action === 'bulkUpdate' ? ACTION_ICONS.bulk
        : action.action === 'validateCP' ? ACTION_ICONS.validate
        : ACTION_ICONS.update;

    const colorClass = action.action === 'addItem' ? 'bg-emerald-50/80 border-emerald-200'
        : action.action === 'removeItem' ? 'bg-red-50/80 border-red-200'
        : action.action === 'bulkUpdate' ? 'bg-amber-50/80 border-amber-200'
        : action.action === 'validateCP' ? 'bg-indigo-50/80 border-indigo-200'
    ...
    return (
        <div className={`flex items-start gap-2 rounded-lg px-3 py-2 text-xs border ${colorClass}`}>
            <span className="mt-0.5 shrink-0">{icon}</span>
            <div className="min-w-0 flex-1">
                <p className="font-medium text-gray-800">{label}</p>
                {targetStr && (
                    <p className="text-[10px] text-gray-400 truncate mt-0.5" title={targetStr}>{targetStr}</p>
                )}
...
function actionLabel(action: CpChatAction): string {
    const labels: Record<string, string> = {
        addItem: 'Agregar Item',
        updateItem: 'Modificar Item',
        removeItem: 'Eliminar Item',
        bulkUpdate: 'Actualizacion Masiva',
        suggestControls: 'Sugerir Controles',
        validateCP: 'Validar Plan de Control',
    };
    return labels[action.action] || action.action;
}

function formatTarget(action: CpChatAction): string {
    if (!action.target) return '';
    const parts: string[] = [];
    if (action.target.processDescription) parts.push(action.target.processDescription);
    if (action.target.index) parts.push(`#${action.target.index}`);
    if (action.target.filter) parts.push(`Filtro: ${action.target.filter}`);
    return parts.join(' | ');
}
...
function ExecutionSummary({ result }: { result: CpChatExecutionResult }) {
        ...
        <div className={`rounded-lg p-3 text-xs border ${
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-gray-800">
                    ...
                    {hasWarnings ? ` \u2022 ${result.warnings.length} advertencia${result.warnings.length !== 1 ? 's' : ''}` : ''}
                </span>
                <button
            ...
            {showDetails && (result.created.length > 0 || result.modified.length > 0 || result.removed.length > 0 || result.warnings.length > 0 || result.errors.length > 0) && (
                <div className="mt-2 space-y-0.5 pl-5">
                    ...
                    {result.removed.map((r, i) => (
                        <p key={`r-${i}`} className="text-red-600">- {r}</p>
                    ))}
...
const EXAMPLE_PROMPTS = [
    'Agrega un item para inspección de torque en ensamble',
    'Cambia la frecuencia a cada hora en todos los items de soldadura',
    'Sugiere controles para el item con AP Alto',
    'Revisa que todos los items tengan plan de reacción completo',
];
...
const CpChatPanel: React.FC<CpChatPanelProps> = ({ doc, amfeDoc, onApplyChanges, onClose, onSettingsChanged }) => {
    ...
    const handleSend = useCallback(async (overrideText?: string) => {
        ...
        try {
            const { response, history } = await sendCpChatMessage(
                text,
                doc,
    ...
    }, [input, isLoading, doc, amfeDoc]);

    const handleApply = useCallback((msgId: string) => {
        const msg = messages.find(m => m.id === msgId);
        if (!msg?.actions) return;

        const result = executeCpChatActions(msg.actions, doc);
                ...
                <div className="flex items-center gap-3 px-5 py-3.5 border-b border-gray-200 bg-gradient-to-r from-emerald-50 via-green-50 to-teal-50">
                    ...
                    <div className="flex items-center gap-1">
                        <button
                            ...
                            className={`p-1.5 rounded-lg transition ${showSettings ? 'text-emerald-600 bg-emerald-100' : 'text-gray-400 hover:text-gray-600 hover:bg-white/60'}`}
                                ...
                                <div className={geminiEnabled ? '' : 'opacity-50 pointer-events-none'}>
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            placeholder="AIzaSy..."
                                            className="w-full px-2.5 py-1.5 pr-8 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400 transition font-mono"
                                        />
                                    ...
                                    <p className="mt-1 text-[10px] text-gray-400">
                                        Obtené tu key en{' '}
                                        <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" className="text-emerald-500 hover:text-emerald-700 underline inline-flex items-center gap-0.5">
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
                    ...
                    {messages.length === 0 && !isLoading && (
                        <div className="flex flex-col items-center justify-center h-full py-8">
                            <div className="w-16 h-16 bg-gradient-to-br from-emerald-100 to-green-100 rounded-2xl flex items-center justify-center mb-4">
                                <ClipboardCheck size={28} className="text-emerald-400" />
                            </div>
                            <p className="text-sm font-semibold text-gray-700">Copiloto Plan de Control</p>
                            <p className="text-xs text-gray-400 mt-1 max-w-xs text-center">
                                Escribí instrucciones en lenguaje natural para modificar tu Plan de Control.
                                La IA propone cambios y vos decidís si aplicarlos.
                            </p>
                            {hasAmfe && (
                                <p className="text-[10px] text-emerald-500 mt-1">
                                    AMFE vinculado disponible para contexto cruzado
                                </p>
                            )}
                            <div className="mt-5 flex flex-wrap gap-2 max-w-md justify-center">
                                {EXAMPLE_PROMPTS.map((prompt, i) => (
                                    <button
                                        ...
                                        className="text-[11px] text-emerald-600 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-full px-3 py-1.5 transition whitespace-nowrap"
                                    >
                                        {prompt}
                                    </button>
                                ))}
                    ...
                    {messages.map(msg => (
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            ...
                            {/* User messages */}
                            {msg.role === 'user' && (
                                <div className="max-w-[80%] bg-gradient-to-br from-emerald-600 to-green-600 text-white rounded-2xl rounded-br-md px-4 py-2.5 shadow-sm">
                ...
                <div className="border-t border-gray-200 px-4 py-3 bg-gray-50/80">
                    <div className="flex items-end gap-2">
                        <textarea
                            ...
                            rows={2}
                            className="flex-1 resize-none text-xs border border-gray-300 rounded-xl px-3.5 py-2.5 focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400 transition placeholder:text-gray-400"
                            disabled={isLoading}
                        />
                        <button
                            onClick={() => handleSend()}

 ...
 * Phase C: Column visibility via visibleColumns prop (backward compatible).
 */
...
const PfdTableRow: React.FC<Props> = ({ step, index, totalSteps, onUpdate, onBatchUpdate, onRemove, onMove, onInsertAfter, onDuplicate, readOnly, visibleColumns, hasBrokenLink, inheritanceStatus }) => {
    ...
    /** Phase C: Column-key-based cell renderers */
    const renderCell = (col: PfdColumnDef): React.ReactNode => {
        switch (col.key) {
            case 'stepNumber': {
                const isTransport = step.stepType === 'transport';
                return (
                    <td key={col.key} className={`${cellClass} text-center font-mono font-bold sticky left-0 z-10 ${stickyBg}`}>
                        <input
                            value={step.stepNumber}
                            onChange={handleTextChange('stepNumber')}
                            className={`${inputClass} text-center font-bold ${isTransport ? 'text-gray-400 italic' : ''}`}
                            placeholder={isTransport ? '' : 'OP 10'}
                            readOnly={readOnly || isTransport}
                            tabIndex={readOnly || isTransport ? -1 : 0}
                        />
                    </td>
                );
            }
            case 'stepType':
                return (
                    <td key={col.key} className={`${cellClass} text-center sticky left-[80px] z-10 ${stickyBg}`}>
                        <PfdSymbolPicker
                            value={step.stepType}
            ...
            case 'description':
                return (
                    <td key={col.key} className={`${cellClass} sticky left-[140px] z-10 ${stickyBg} shadow-[2px_0_4px_rgba(0,0,0,0.06)]`}>
            ...
            case 'branchId':
                return (
                    <td key={col.key} className={`${cellClass} text-center ${step.branchId && branchColor ? branchColor.bg : ''}`}>
                        {readOnly ? (
                            step.branchId ? (
    ...
    return (
        ...
        >
            {columnsToRender.map(col => renderCell(col))}
                ...
                <td className={`px-1 py-1 text-center whitespace-nowrap sticky right-0 z-10 ${stickyBg} shadow-[-2px_0_4px_rgba(0,0,0,0.06)]`}>
                    <div className="flex items-center gap-px justify-center">
                        <button
                            ...
                            disabled={index === 0}
                            className="p-0.5 text-gray-400 hover:text-cyan-600 disabled:opacity-50 disabled:cursor-not-allowed transition rounded focus-visible:ring-2 focus-visible:ring-cyan-300"
                            title="Mover arriba"
                        >
                            <ArrowUp size={13} />
                        </button>
                        ...
                        {onInsertAfter && (
                            <button
                                ...
                                className="p-0.5 text-gray-400 hover:text-green-600 transition rounded focus-visible:ring-2 focus-visible:ring-green-300"
                        ...
                        {onDuplicate && (
                            <button
                                ...
                                className="p-0.5 text-gray-400 hover:text-blue-600 transition rounded focus-visible:ring-2 focus-visible:ring-blue-300"
                                title="Duplicar paso"
                            >
                                <Copy size={13} />
                            </button>
                        ...
                        <button
                            ...
                            className="p-0.5 text-gray-400 hover:text-red-500 transition rounded focus-visible:ring-2 focus-visible:ring-red-300"
                            title="Eliminar paso"
                        >
                            <Trash2 size={13} />
                        </button>
...
export default React.memo(PfdTableRow);

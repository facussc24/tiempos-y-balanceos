> Archivo historico completo (2026-03-30 a 2026-07-02), archivado 2026-07-03. La version viva destilada esta en docs/LECCIONES_APRENDIDAS.md.
...
## 2026-07-02 — AMFE IP PAD alineado a flujograma/HO/BOM; editar Supabase sin .env.local

**Contexto:** Fak paso carpeta "Enviar a Marcelo Nieve" (flujograma PNG, HO-985 PDF, BOM006 xlsx,
Antwortbogen INCA). Pidio alinear el AMFE IP PAD (`VWA-PAT-IPPADS-001`, id c9b93b84) a esas fuentes,
auditar todo, resolver el/los codigo(s) mal, y dejar el AMFE exportado a Excel en la carpeta.

**Lecciones:**
1. **Codigo L2 correcto = GKX**, no GKK. El AMFE tenia GKK (mal); flujograma+HO+`pfd.md` dicen GKX.
   El BOM006 tambien quedo con GKK (aviso pendiente a Fak). Cuando Fak dice "el codigo esta mal",
   la fuente de verdad son los docs nuevos que el mismo hizo (flujograma+HO), no el dato viejo.
2. **Esta PC (`C:\Dev\BarackMercosul`) NO tiene `.env.local`** → no corre `_backup.mjs` ni
   `runWithValidation`. Se edita via MCP Supabase `execute_sql` (rol postgres, bypassa el RLS que es
   authenticated-only). Backup = tabla `_backup_ippad_<fecha>` via `CREATE TABLE AS`. Patron de
   escritura seguro: cirugia jsonb por lotes chicos + verificacion pre/post por SQL. Recalcular AP
   con funcion PL/pgSQL que replica `calculateAP` (apTable.ts), nunca formula S*O*D.
3. **Severidad = efecto en el USUARIO, no el scrap** (error conceptual que infla S). Costura
   "descosida/desviada" con efecto estetico va S5-6, no S8. El scrap sube ocurrencia-costo, no
   severidad. Corregido en OP40/41/42.
4. **AIAG-VDA S9-10 SI cubre seguridad del OPERARIO** (OS). Cortadura/quemadura/ventilacion con S10
   estan BIEN sin marcar CC. No "corregir" eso a la baja.
5. **O=10 con controles declarados es indefendible** (O=10 = "sin control alguno"). Un error humano
   ocasional con instruccion+autocontrol = O~6 (control conductual). Corregido en OP120.
6. **Regla 1M por linea**: separar WE agrupados ("Herramientas manuales + Dispositivo de ultrasonido"
   → 2 WEs) repartiendo las fallas por semantica del recurso.
7. **Export Excel oficial via `scripts/_exportIpPad.ts`** (variante de `_exportOficial.ts`, usa
   `buildAmfeCompletoWorkbook`): lee el archivo persistido del resultado MCP y escribe a la carpeta.
8. **Bug propio:** genere SQL con `part_number = "..."` (comillas dobles = identificador en Postgres).
   Los string literals van con comilla simple. Verificar antes de correr UPDATEs generados.
9. Resolver calibraciones dudosas con **multiples agentes contra el manual** en vez de rebotarle la
   pregunta a Fak ("no me estas preguntando si en mi planta pasa esto, es metodologia AIAG-VDA").

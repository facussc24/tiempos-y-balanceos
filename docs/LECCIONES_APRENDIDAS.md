...
- **13/08 — Mi propia verificacion se aborto sola por error de muestreo, dos veces.** Vecino-mas-cercano con densificado de 1,0 mm daba 1,05 mm de falsa asimetria entre manos, y con 0,5 mm daba 0,41 mm de falsa desviacion tras simplificar; los valores reales eran 0,05. **Si el resultado baja proporcional al paso, se esta midiendo el muestreo.** Densificado fino y umbral atado al paso.
...
- **13/08 — La revision de un plano esta en el CAJETIN, no en el nombre.** Busque `*R1*` por nombre en todo el PPAP y casi informo que las plantillas de Fak "no tenian precedente"; los PDF sin sufijo decian **REVISION: 1** con numero de plano y firma. **Un `find` no ve adentro del archivo**: antes de afirmar que algo no tiene antecesor, ABRIRLO.

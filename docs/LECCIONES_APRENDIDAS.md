...
- **13/08 — Dos veces en la misma sesion la respuesta ya estaba escrita en mi propia skill, y no la aplique.** (a) Arme dos hojas con cruces X cuando el registro decia que la X no sirve para cuchilla de arrastre; (b) escribi el mix pieza-por-pieza (contorno y despues sus agujeros) cuando la skill dice literal "los agujeros se cortan ANTES del contorno, despues la pieza ya esta suelta y el agujero se va" — Fak lo cazo cortando: *"algunos circulos el plotter no los esta haciendo"*. **Leer la skill del area ANTES de escribir el codigo, no despues de que falle**; y cuando una regla es de orden o de secuencia, el codigo tiene que **releer el archivo escrito y abortar** si no se cumple, porque "lo escribi en el orden correcto" no se ve en el resultado. Ambas quedaron con enforcement en `_mixPlotter.py`.

- **13/08 — La revision de un plano esta en el CAJETIN, no en el nombre del archivo.** Busque `*R1*` por nombre en todo el PPAP, no aparecio, y estuve a punto de informar que las plantillas laser R2 de Fak "no tenian precedente". Los abri: los PDF sin sufijo dicen **REVISION: 1**, con numero de plano BCK-0SEWIT/BCK-0SEWID, realizo/aprobo y fecha. **Un `find` por nombre no ve adentro del archivo**: antes de afirmar que algo no tiene antecesor, ABRIR los documentos parecidos y leer el cajetin. Vale para revisiones, para saber que reemplaza que, y para no archivar lo que no corresponde.
- **13/08 — Un chequeo que grita "no coincide" puede estar midiendo un cambio deliberado.** Los agujeros de la plantilla nueva caian a 56 mm de los del patron; mostre la tabla y pregunte en vez de "corregir": Fak los habia movido el. **El chequeo frena, no decide.** Antes de eso arme dos comparaciones invalidas (un split que dejo un agujero en X negativa, un cotejo de bbox entre objetos distintos): **una comparacion que no puedo justificar geometricamente no es evidencia**. La valida fue la de separaciones entre agujeros, invariante a traslacion.
- **13/08 — Un chequeo de simetria con muestreo grueso INVENTA asimetria.** El espejo izq/der daba 1,05 mm con densificado 1,0 mm y parecia un patron torcido; con 0,05 da 0,05. **Si el resultado baja proporcional al paso, lo que se mide es el muestreo, no la pieza** — el umbral va atado al paso, y reportar eso como hallazgo mandaba a revisar un patron que estaba perfecto.

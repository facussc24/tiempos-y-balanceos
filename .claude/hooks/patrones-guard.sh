#!/usr/bin/env bash
# patrones-guard.sh — wrapper fino. Desde el 05/09/2026 la logica vive en
# scripts/_lib/guardianes.mjs (guardian "patrones-guard"), junto con la de los otros doce: el
...
# Antes: patrones-guard.sh — RECORDATORIO (1x por hora) de los 3 gates de patrones de corte.
#
...
#   printf '%s' "$JSON" | bash .claude/hooks/patrones-guard.sh      # exit 0 pasa · exit 2 bloquea
...
exec node "$RAIZ/scripts/_lib/guardianes.mjs" --solo patrones-guard

#!/usr/bin/env bash
# mail-guard.sh — wrapper fino. Desde el 05/09/2026 la logica vive en
# scripts/_lib/guardianes.mjs (guardian "mail-guard"), junto con la de los otros doce: el
...
# Antes: Mail Guard Hook (PreToolUse, matcher = Bash|PowerShell|Write|Edit)
#
...
#   printf '%s' "$JSON" | bash .claude/hooks/mail-guard.sh      # exit 0 pasa · exit 2 bloquea
...
exec node "$RAIZ/scripts/_lib/guardianes.mjs" --solo mail-guard

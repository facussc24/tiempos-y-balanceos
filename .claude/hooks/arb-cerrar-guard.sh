#!/usr/bin/env bash
# arb-cerrar-guard.sh — wrapper fino. Desde el 05/09/2026 la logica vive en
# scripts/_lib/guardianes.mjs (guardian "arb-cerrar-guard"), junto con la de los otros doce: el
...
# Antes: arb-cerrar-guard.sh — Enforcement de la regla `arb-no-cerrar.md`:
#
...
#   printf '%s' "$JSON" | bash .claude/hooks/arb-cerrar-guard.sh      # exit 0 pasa · exit 2 bloquea
...
exec node "$RAIZ/scripts/_lib/guardianes.mjs" --solo arb-cerrar-guard

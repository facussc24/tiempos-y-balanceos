#!/usr/bin/env bash
# cad-guard.sh — wrapper fino. Desde el 05/09/2026 la logica vive en
# scripts/_lib/guardianes.mjs (guardian "cad-guard"), junto con la de los otros doce: el
...
# Antes: cad-guard.sh — RECORDATORIO (1x por hora) de los 2 gates 3D del skill cad-design.
#
...
#   printf '%s' "$JSON" | bash .claude/hooks/cad-guard.sh      # exit 0 pasa · exit 2 bloquea
...
exec node "$RAIZ/scripts/_lib/guardianes.mjs" --solo cad-guard

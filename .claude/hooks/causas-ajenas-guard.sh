#!/usr/bin/env bash
# causas-ajenas-guard.sh — wrapper fino. Desde el 05/09/2026 la logica vive en
# scripts/_lib/guardianes.mjs (guardian "causas-ajenas-guard"), junto con la de los otros doce: el
...
# Antes: Guardian PreToolUse — bloquea RECONSTRUCCIONES CAUSALES de errores ajenos sin fuente.
#
...
#   printf '%s' "$JSON" | bash .claude/hooks/causas-ajenas-guard.sh      # exit 0 pasa · exit 2 bloquea
...
exec node "$RAIZ/scripts/_lib/guardianes.mjs" --solo causas-ajenas-guard

#!/usr/bin/env bash
# ho-numeracion-guard.sh — wrapper fino. Desde el 05/09/2026 la logica vive en
# scripts/_lib/guardianes.mjs (guardian "ho-numeracion-guard"), junto con la de los otros doce: el
...
# Antes: ho-numeracion-guard.sh — Enforcement de la regla no-pfd-no-ho §"la numeracion
#
...
#   printf '%s' "$JSON" | bash .claude/hooks/ho-numeracion-guard.sh      # exit 0 pasa · exit 2 bloquea
...
exec node "$RAIZ/scripts/_lib/guardianes.mjs" --solo ho-numeracion-guard

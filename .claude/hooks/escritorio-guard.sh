#!/usr/bin/env bash
# escritorio-guard.sh — wrapper fino. Desde el 05/09/2026 la logica vive en
# scripts/_lib/guardianes.mjs (guardian "escritorio-guard"), junto con la de los otros doce: el
...
# Antes: escritorio-guard.sh — protege la cola de tareas y su archivo.
#
...
#   printf '%s' "$JSON" | bash .claude/hooks/escritorio-guard.sh      # exit 0 pasa · exit 2 bloquea
...
exec node "$RAIZ/scripts/_lib/guardianes.mjs" --solo escritorio-guard

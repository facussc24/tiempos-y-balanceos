...
`tmp/` (dumps de inspeccion) · `backups/` · las auditorias viejas de `docs/_archive/` · datos hardcodeados en scripts viejos.
...
## Protocolo
- Pregunta sobre estado actual → query primero, responder despues. Script temporal `scripts/_tmp_query_*.mjs` (borrar despues, no commitear) con `.env.local` (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, login con `VITE_AUTO_LOGIN_*`).
